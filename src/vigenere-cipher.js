class VigenereCipheringMachine {
  ...
  encrypt(message, key, direct = true) {
    ...
    function toCode(word) {
      ...
      return wordToArr.map(symbol => (symbol.charCodeAt(0) >= FIRST_LETTER_UNICODE && symbol.charCodeAt(0) <= LAST_LETTER_UNICODE) ? symbol.charCodeAt(0) : symbol);
    }
    ...
    for (let char of messageCode) {
      if (typeof char !== 'number') {
        ...
        if (direct) {
          ...
          (char - keyCodeChar) >= 0 ? displacement = (char - keyCodeChar) : displacement = ALPHABET_SIZE + (char - keyCodeChar);
        }
